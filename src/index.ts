...
function printBanner(): void {
  process.stderr.write(`\n  ${green}${bold}\u2714 Mnemexa MCP Adapter${reset} ${dim}v${ADAPTER_VERSION}${reset}\n`);
...
function printNotConnected(): void {
  ...
  process.stderr.write(`  ${cyan}${bold}"Install Mnemexa memory, my API key is <your-key>"${reset}\n\n`);
  process.stderr.write(`  ${dim}Get your key at${reset} ${magenta}https://app.mnemexa.com${reset}\n\n`);
}
...
async function main(): Promise<void> {
  ...
  // Interactive terminal (user ran npx @mnemexa/mcp directly)
  ...
  process.on("SIGINT", () => {
    process.stderr.write(`\n  ${dim}Mnemexa MCP Adapter shutting down${reset}\n`);
    process.exit(0);
  });
...
main().catch(() => {
  process.stderr.write(`\n  ${yellow}Mnemexa MCP Adapter failed to start.${reset}\n`);
  process.exit(1);
});
