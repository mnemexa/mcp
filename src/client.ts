...
const DEFAULT_BASE_URL = "https://api.mnemexa.com";
...
export class BizXClient {
  ...
  private resolveBaseUrl(): string {
    const override = process.env.MNEMEXA_BASE_URL || process.env.BIZX_BASE_URL;
    if (!override) return DEFAULT_BASE_URL;

    const url = override.replace(/\/+$/, "");
    const isDev = process.env.NODE_ENV === "development";
    const isOfficialDomain =
      url.startsWith("https://api.mnemexa.com") ||
      url.startsWith("https://api.bizxengine.com");
